#!/usr/bin/env python3
"""
避坑指南图文生成 - 执行任务接口（真实调用时使用）
调用方式: python scripts/execute_task.py --question "夜市品尝小吃"
...
def execute_task(
    ...
    Args:
        question: 用户目标事项，以此作为图文主题与内容核心（必填）
        aspect_ratio: 输出画幅比例，可选值：3:4(小红书/海报)、16:9(视频封面)、1:1(朋友圈)
    ...
    # 2. 构建请求
    url = "https://route.showapi.com/flow/execute/6a7003ca3c51f2ec21006edd"
    headers = {
...
def main():
    parser = argparse.ArgumentParser(description="避坑指南图文生成 - 执行任务")
    parser.add_argument("--question", "-q", required=True, help="用户目标事项，以此作为图文主题与内容核心")
    parser.add_argument("--aspect-ratio", "-a", default="3:4", choices=["3:4", "16:9", "1:1"], help="输出画幅比例，可选值：3:4(小红书/海报)、16:9(视频封面)、1:1(朋友圈)")
