...
示例:
    python3 merge_research.py ~/.workbuddy/skills/elon-musk-perspective
...
def extract_key_findings(content: str, max_items: int = 3) -> list[str]:
    """提取关键发现（优先「关键发现」section的加粗结论，其次##标题）"""
    # 优先：从「## 关键发现」section 提取加粗结论（与调研文件统一格式对齐）
    kf = re.search(r'##\s+关键发现\s*(.*?)(?=\n##\s|\Z)', content, re.DOTALL)
    if kf:
        bolds = re.findall(r'\*\*(.+?)\*\*', kf.group(1))
        if bolds:
