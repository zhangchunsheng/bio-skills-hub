...
用法(遵循技能环境约定,先用 ensure_env.py 拿到 venv 解释器,再运行本脚本):
    ...
    性能优化(v2.5.7):15 个验证脚本彼此独立、产物前缀互不冲突,默认**并行**执行
...
VERIFY_SCRIPTS = [
    ...
    "verify12_combined_panel.py", # v2.7.0 新增：多数据集合并面板图 + 总报告（compose_panel_figure / build_master_report）
    "verify13_multi_sheet.py",     # v2.7.2 新增：单 Excel 多 sheet 读取 → 合并面板图（read_excel_sheets）
]
...
PRESETS = {
    "stats": [
        ...
        "verify13_multi_sheet.py",     # v2.7.2 单 Excel 多 sheet 读取 → 合并面板图
    ],
    "plot": [
        ...
        "verify12_combined_panel.py",     # v2.7.0 多数据集合并面板图 + 总报告
    ],
...
def main():
    ...
    if not selected:
        # 默认（无 --only/--quick）跑全套注册脚本，与 README/文档一致
        selected = VERIFY_SCRIPTS
