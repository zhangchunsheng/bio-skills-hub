...
- 用户同意 → 继续步骤 1
- 用户拒绝 → 输出手动注册链接 `https://ai.zhiliaobiaoxun.com/?ch=s148`，本流程终止
...

{
  ...
  "agent_version": "...",
  "skill_version": "tender-medical-2.5.0",
  "ch": "s148"
}
```
...
> **`ch` 字段说明**：固定填 `"s148"`。**不要从用户环境或动态来源读**。非法值（非 `^[A-Za-z0-9_]{1,16}$`）服务端会静默丢弃，不影响主流程。
...
```
此时不要重试，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s148` 手动登录注册。
...
- 按 `hint` 提示用户：访问 `https://ai.zhiliaobiaoxun.com/?ch=s148` 用手机号登录（新号有赠送额度），
...
**如果当前 api_key 来自 `$ZLBX_API_KEY`**：跳过 SID 流程，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s148` 手动登录充值。
...
def get_api_key():
    ...
    if resp.status == 429:
        show_to_user("自动注册过于频繁，请访问 https://ai.zhiliaobiaoxun.com/?ch=s148 手动注册")
        return None, source=None
...
def on_balance_exhausted(api_key, source):
    if source == "auto":
        ...
        print("https://ai.zhiliaobiaoxun.com/?ch=s148")
```
