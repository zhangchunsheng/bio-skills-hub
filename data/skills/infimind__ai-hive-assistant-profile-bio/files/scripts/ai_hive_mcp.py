...
def redact(text):
    for name in ("AI_HIVE_API_KEY", "AI_HIVE_ACCESS_TOKEN"):
...
def auth_headers():
    ...
    value = token or key
    if not value:
        raise ClientError("未提供凭据。请在支持 OAuth 的 MCP 客户端登录，或通过 Secret/环境变量提供 AI_HIVE_API_KEY；doctor 不需要凭据。")
    if any(c in value for c in ("\r", "\n")):
        raise ClientError("凭据包含非法换行，请检查 Secret 配置。")
