#!/usr/bin/env python3
"""
生物本体映射器（集成UMLS/MeSH API）
将非结构化生物医学文本映射到SNOMED CT和MeSH本体。
使用官方UMLS UTS API和MeSH API进行准确的术语映射。
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='将生物医学术语映射到SNOMED CT和MeSH本体'
    )
    parser.add_argument('--term', type=str, help='要映射的单个术语')
    ...
    parser.add_argument('--use-api', action='store_true',
                       help='使用UMLS/MeSH API')
    parser.add_argument('--api-key', type=str,
                       help='UMLS API密钥（或设置UMLS_API_KEY环境变量）')
