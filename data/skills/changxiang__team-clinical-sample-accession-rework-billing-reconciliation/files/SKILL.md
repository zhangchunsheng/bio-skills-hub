---
name: team-clinical-sample-accession-rework-billing-reconciliation
description: "Reconcile clinical-trial sample-accession rework billing across studies, sites, pseudonymized specimens and requisitions, collection and shipment, receipt conditions, barcodes and accession entries, missing-data and mismatch queries, relabeling, reaccession and data corrections, failure reasons, turnaround time, rates, invoices, credits, payments, and trial ledgers without reidentifying participants, handling specimens, changing source or laboratory records, deciding sample validity, making clinical decisions, or moving funds. This Pay Skill provides free scope validation, then requires verified payment before releasing the full deliverable."
tags: [临床试验, 样本接收, 实验室返工, 处理费账单]
version: 2.0.2
capability: clinical_sample_accession_rework_billing_reconciliation
pricing:
  model: per_call
  amount_fen: 599
---
...
- 研究中心、去标识样本申请单、采集发运和接收条件
- 条码录入、缺失错配查询、重贴标重接收、数据更正和失败原因
- 周转时长、费率、发票贷项付款和试验总账
...
Price: `CNY 5.99` per paid invocation. Skill version: `2.0.2`. Product ID: `SP2608160406`.
...
3. 按去标识样本与申请单重建发运、接收、查询、返工和更正链
4. 核对返工原因、责任归属、次数、周转时长和合同费率
5. 分类幽灵返工、重复处理、供应商原因转嫁、无查询证据和账簿差异
...
`差异ID | 研究/实验室 | 中心/去标识样本 | 申请单/条码 | 发运/接收条件 | 接收录入 | 缺失/错配查询 | 重贴标/重接收/更正 | 失败原因/责任 | 周转时长 | 费率/发票/差异金额 | 证据 | 状态`
...
- 不得重识别受试者、接触样本或修改源数据与实验室记录
- 不得判断样本有效性、作临床决定或划款
- 身份断链、系统性错配、温控异常或大批量无依据返工必须升级实验室质量、临床运营、数据管理与管理层
