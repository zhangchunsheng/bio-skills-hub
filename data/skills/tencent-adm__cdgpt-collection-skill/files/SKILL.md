...
description: "CD-GPT 生成式生物基础大模型，对 DNA/RNA/蛋白质序列预训练，支持单分子与多分子联合分析的预测与生成。"
triggers: [多分子联合分析的预测, CD-GPT Collection (Nextflow), cd-gpt collection (nextflow), cd-gpt-collection-(nextflow), cd-gpt_collection_(nextflow), cd-gptcollection(nextflow), 单细胞与多组学基础模型, 跑 CD-GPT Collection (Nextflow), run cd-gpt collection (nextflow)]
...
**应用简介**：CD-GPT 生成式生物基础大模型，对 DNA/RNA/蛋白质序列预训练，支持单分子与多分子联合分析的预测与生成。
...
| ---- | ---------- | ---- | --------------- | ----------- | ---- |
| 1 | `CD-GPT_reverse_translation_generation` | `NEXTFLOW` | `0f8efde4-1a1c-4b67-861c-1f03ec15261e` | v24.04.3 | CD-GPT逆向翻译生成 |
| 2 | `CD-GPT_translation_generation` | `NEXTFLOW` | `995632d2-a434-458d-b2dc-19e1759c379b` | v24.04.3 | CD-GPT正向翻译生成 |
...
本 SKILL 仅服务于 `CD-GPT Collection (Nextflow)` 合集（AppId: `5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c`）。
...
> CD-GPT Collection (Nextflow) 包含以下子应用，请选择一个：
>
  1. **CD-GPT_reverse_translation_generation** (`NEXTFLOW` · NF版本: v24.04.3) — CD-GPT逆向翻译生成
  2. **CD-GPT_translation_generation** (`NEXTFLOW` · NF版本: v24.04.3) — CD-GPT正向翻译生成
>
> 请输入编号或应用名称进行选择。
...
- `<selectedAppId>` → 来自上方子应用清单的 AppId，**不得使用合集本身的 AppId（5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c）**
...
> - `<selectedAppId>` 来自本 SKILL 子应用清单，**不得使用合集本身的 AppId（5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c）**
...
### 场景 B：用户直接说出子应用名
用户说「帮我跑 CD-GPT Collection (Nextflow) 里的 XXX」→ Step 3 精确匹配 → 确认 → 同名检查 → 运行
...
**场景：使用 AI 生成的蛋白质序列运行 CD-GPT Collection (Nextflow) 子应用**

```
用户: 生成测试蛋白质序列，使用默认模板运行CD-GPT Collection (Nextflow)
...
Agent:
  ...
  Agent:
    ...
    6. [继续] omics run --public-app 5e3b30ee-85a2-4ad7-80c5-fa2f0ecb0d0c --input /tmp/run_input.json ...
```
