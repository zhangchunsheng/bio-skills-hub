...
# -*- coding: utf-8 -*-
r"""
ensure_env.py — 可移植、幂等的 Python 绘图环境引导器（neuroresearch / prism-style-plot 共用）。

为什么需要它（踩坑记录，分享时务必保留）：
  受管理 Python 的绘图依赖（matplotlib/scipy/numpy/pandas/seaborn/statsmodels）装在
  **受管理 venv** 里，而不在基础解释器上。如果用基础解释器去 `import matplotlib` 做
  "装没装"的判定，会报 ModuleNotFoundError，从而误判为未安装、每次都重装。本脚本只检查
  venv 自己的解释器；首次装好后在 venv 内写入标记文件，之后运行仅做文件系统检查（秒回，
  不再全量 import），并返回 venv 解释器绝对路径，供后续统一调用。传递 --force 可强制
  重新探针（用于 venv 被手动破坏后的恢复）。

可移植性：
  - venv 路径不硬编码用户名，而是从当前解释器 sys.executable 向上推导受管理结构
  ...
  - Windows 受管理 Python 上 `python -m venv` 可能卡在 launcher 重定向，创建 venv 时
    加 --copies 走副本模式兜底（neuroresearch 在 Windows 上的实测坑）。
...
REQUIRED = ["matplotlib", "scipy", "numpy", "pandas", "seaborn",
            "statsmodels"]  # statsmodels 供 build_stats_report("twoway") 的 Two-way ANOVA
...
def main():
    ...
    if not os.path.exists(py):
        ...
        # Windows 受管理 Python 上 `python -m venv` 可能卡在 launcher 重定向：
        # 加 --copies 走副本模式兜底（neuroresearch 在 Windows 上的实测坑）。
        cmd = [sys.executable, "-m", "venv"]
        if os.name == "nt":
            cmd.append("--copies")
        cmd.append(venv_root)
        try:
            subprocess.run(cmd, check=True)
        except subprocess.CalledProcessError as e:
            print("[ensure_env] venv 创建失败（Windows launcher 卡顿常见）："
                  "请按 python-runtime.md §四 用 `--copies` 手动重建，"
                  "或检查网络/镜像源", file=sys.stderr)
