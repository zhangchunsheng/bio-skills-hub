...
# 本文件从独立 prism-style-plot 技能（v2.7.4）同步升级；记录上游版本号，
# 供 neuroresearch 的 selfcheck「引擎双源同步」检查（vendored ↔ 独立
# prism-style-plot 技能）做数量/版本比对，防止拷贝静默落后。
UPSTREAM_VERSION = "v2.7.4"
UPSTREAM_SYNCED = "2026-08-26"   # 最近一次自上游同步的日期（回溯/对账用）
...
def _auto_stat_method(test):
    ...
    stat_method 就会写出错误的 ANOVA 描述（已在 2 组示例图中实测踩坑）。
...
def read_table(path, group_col=None, value_col=None, sheet=0, header=0):
    ...
    **当数据来自文件、且用户未显式指定 y 轴标签时，自动用数值列的
    ...
    自动判别表型：
      - 长格式（long）：恰好一列非数值（分组列）+ ≥1 列数值。
        例：group=Control/Treated，值为 Relative TargetX expression
    ...
    参数：
        path       : CSV 或 Excel（.xlsx/.xls/.xlsm）文件路径
        group_col  : 长格式的分组列名/索引；None = 自动选唯一非数值列
        value_col  : 长格式的数值列名/索引；None = 自动选数值列
    ...
    # 长格式：恰好一列非数值（分组）+ ≥1 数值列 → value 列表头即 ylabel
    if len(non_numeric_cols) == 1 and len(numeric_cols) >= 1:
        gcol = group_col if group_col is not None else non_numeric_cols[0]
        vcol = value_col if value_col is not None else numeric_cols[0]
        groups = [g[vcol].dropna().astype(float).tolist()
                  for _, g in df.groupby(gcol, sort=False)]
        labels = [str(l) for l in df[gcol].drop_duplicates().tolist()]
        ylabel = _clean(vcol)
        fmt = "long"
    else:
        # 宽格式：每列一组（无单一数值列名，ylabel 回退 None）
        groups = [df[c].dropna().astype(float).tolist() for c in cols]
        labels = [_clean(c) for c in cols]
        ylabel = None
        fmt = "wide"
...
def read_excel_sheets(path, header=0, sheets=None, skip_empty=True):
    ...
    **用于"用户把多组数据放在同一 Excel 的不同 sheet"的场景**：一次调用
    ...
    # 索引 → 真实 sheet 名
    names = [xls.sheet_names[s] if isinstance(s, int) else s for s in raw]
    ...
    for sname in names:
        ...
        if skip_empty and df.dropna(how="all").empty:
            continue
        info = read_table(path, sheet=sname, header=header)
        info["sheet"] = sname
        info["source"] = f"{path}::{sname}"
        out.append(info)
    return out
