...
def build_zn_hcl():
    return {
        "meta": {"title": "锌与盐酸反应", "subtitle": "Zn + 2HCl → ZnCl₂ + H₂↑",
                 "language": "zh-CN", "category": "inorganic", "accent": "indigo", "engine": "morph"},
        "conditions": {"text": "常温", "exothermic": True, "transitionGlow": True},
        "reactants": [{"species": "Zn", "count": 1, "pos": [-4.0, 0.2, 0]},
                      {"species": "HCl", "count": 2}],
        "products": [{"species": "ZnCl2", "count": 1, "pos": [-2.5, 1.0, 0]},
                     {"species": "H2", "count": 1}],
        "atom_map": [
            ["Zn#1.X", "ZnCl2#1.Y"],
            ["HCl#1.H", "H2#1.Ha"], ["HCl#1.Cl", "ZnCl2#1.Xa"],
            ["HCl#2.H", "H2#1.Hb"], ["HCl#2.Cl", "ZnCl2#1.Xb"],
        ],
        "electrons": [
            {"from": "Zn1", "to": "H1", "count": 1, "label": "e⁻"},
            {"from": "Zn1", "to": "H2", "count": 1, "label": "e⁻"},
        ],
        "energy": {"activation": 0.3, "deltaH": -0.5,
                   "reactantLabel": "Zn+HCl", "productLabel": "ZnCl₂+H₂"},
        "steps": [
            {"title": "锌片投入盐酸",
             "html": "<span class='text-slate-300 font-semibold'>锌 (Zn)</span> 是活泼金属，"
                     "<span class='text-green-400 font-semibold'>盐酸 (HCl)</span> 是强酸。"
                     "锌表面遇到 H⁺ 离子，开始发生<span class='text-indigo-400 font-semibold'>氧化还原反应</span>。"},
            {"title": "电子转移 · 产生氢气",
             "html": "Zn 失去 2 个 <span class='text-cyan-300 font-semibold'>电子 (e⁻)</span> 变成 Zn²⁺（<span class='text-amber-400 font-semibold'>氧化</span>），"
                     "2 个 H⁺ 各得到 1 个电子变成 <span class='text-sky-300 font-semibold'>H 原子</span>，"
                     "结合成 <span class='text-sky-300 font-semibold'>H₂ 气泡</span> 逸出。"},
            {"title": "生成氯化锌与氢气",
             "html": "Zn²⁺ 与 2 个 Cl⁻ 结合成 <span class='text-indigo-300 font-semibold'>ZnCl₂</span> 溶液，"
                     "<span class='text-sky-300 font-semibold'>H₂</span> 以气泡形式放出。"
                     "电子转移 2e⁻，原子守恒：1Zn·2H·2Cl。"},
        ],
    }
...
def build_na_water():
    return {
        "meta": {"title": "钠与水反应", "subtitle": "2Na + 2H₂O → 2NaOH + H₂↑",
                 "language": "zh-CN", "category": "inorganic", "accent": "violet", "engine": "morph"},
        "conditions": {"text": "常温", "exothermic": True, "transitionGlow": True},
        "reactants": [{"species": "Na", "count": 2, "pos": [-4.5, 0.5, 0]},
                      {"species": "H2O", "count": 2}],
        "products": [{"species": "NaOH", "count": 2, "pos": [-2.0, 1.2, 0]},
                     {"species": "H2", "count": 1}],
        "atom_map": [
            ["Na#1.X", "NaOH#1.Na"], ["Na#2.X", "NaOH#2.Na"],
            ["H2O#1.A", "NaOH#1.A"], ["H2O#2.A", "NaOH#2.A"],
            ["H2O#1.Ha", "NaOH#1.H"], ["H2O#2.Ha", "NaOH#2.H"],
            ["H2O#1.Hb", "H2#1.Ha"], ["H2O#2.Hb", "H2#1.Hb"],
        ],
        "electrons": [
            {"from": "Na1", "to": "H3", "count": 1, "label": "e⁻"},
            {"from": "Na2", "to": "H4", "count": 1, "label": "e⁻"},
        ],
        "energy": {"activation": 0.15, "deltaH": -0.85,
                   "reactantLabel": "Na+H₂O", "productLabel": "NaOH+H₂"},
        "steps": [
            {"title": "钠投入水中",
             "html": "<span class='text-violet-300 font-semibold'>钠 (Na)</span> 极度活泼，遇水立即反应。"
                     "钠的密度比水小，浮在水面，熔化成小球并在水面<span class='text-amber-400 font-semibold'>剧烈游动</span>。"},
            {"title": "钠失电子 · 水得电子",
             "html": "每个 Na 失去 1 个 <span class='text-cyan-300 font-semibold'>电子</span> 变成 Na⁺（<span class='text-amber-400 font-semibold'>氧化</span>），"
                     "H₂O 中的 H（+1 价）得到电子变成 <span class='text-sky-300 font-semibold'>H 原子</span>，"
                     "两两结合成 H₂ 放出，反应剧烈放热。"},
            {"title": "生成氢氧化钠与氢气",
             "html": "Na⁺ 与 OH⁻ 结合成 <span class='text-violet-300 font-semibold'>NaOH（氢氧化钠）</span>，"
                     "<span class='text-sky-300 font-semibold'>H₂</span> 气泡逸出。"
                     "电子转移 2e⁻，原子守恒：2Na·4H·2O。"},
        ],
    }
...
def build_mg_burn():
    return {
        "meta": {"title": "镁条燃烧", "subtitle": "2Mg + O₂ 点燃生成 MgO，发出耀眼白光",
                 "language": "zh-CN", "category": "inorganic", "accent": "amber", "engine": "morph"},
        "conditions": {"text": "点燃", "exothermic": True, "flame": True},
        "reactants": [{"species": "Mg", "count": 2, "pos": [-4.0, 0.2, 0]},
                      {"species": "O2", "count": 1}],
        "products": [{"species": "MgO", "count": 2}],
        "atom_map": [
            ["Mg#1.X", "MgO#1.Mg"], ["Mg#2.X", "MgO#2.Mg"],
            ["O2#1.Oa", "MgO#1.O"], ["O2#1.Ob", "MgO#2.O"],
        ],
        "electrons": [
            {"from": "Mg1", "to": "O1", "count": 2, "label": "e⁻"},
            {"from": "Mg2", "to": "O2", "count": 2, "label": "e⁻"},
        ],
        "energy": {"activation": 0.55, "deltaH": -0.9,
                   "reactantLabel": "Mg+O₂", "productLabel": "MgO"},
        "steps": [
            {"title": "点燃镁条",
             "html": "<span class='text-slate-300 font-semibold'>镁条 (Mg)</span> 在空气中点燃，"
                     "达到着火点后剧烈燃烧，发出<span class='text-amber-300 font-semibold'>耀眼白光</span>。"
                     "镁是活泼金属，常温下表面有氧化膜，点燃后剧烈反应。"},
            {"title": "电子转移 · 剧烈燃烧",
             "html": "Mg 失去 2 个 <span class='text-cyan-300 font-semibold'>电子</span> 变成 Mg²⁺（<span class='text-amber-400 font-semibold'>氧化</span>），"
                     "O 得到 2 个电子变成 O²⁻（<span class='text-emerald-400 font-semibold'>还原</span>）。"
                     "反应放出大量热与光——<span class='text-amber-300 font-semibold'>燃烧</span>的微观本质是剧烈的氧化还原。"},
            {"title": "生成氧化镁",
             "html": "Mg²⁺ 与 O²⁻ 通过<span class='text-amber-300 font-semibold'>离子键</span>结合成"
                     "<span class='text-slate-200 font-semibold'>氧化镁 (MgO)</span>——白色粉末。"
                     "电子守恒：Mg 共失 4e⁻、O 共得 4e⁻ ✓。原子守恒：2Mg·2O。"},
        ],
    }
...
def build_fe_cuso4():
    return {
        "meta": {"title": "铁钉置换铜", "subtitle": "Fe + CuSO₄ → FeSO₄ + Cu",
                 "language": "zh-CN", "category": "inorganic", "accent": "emerald", "engine": "morph"},
        "conditions": {"text": "常温", "exothermic": True, "transitionGlow": True},
        "reactants": [{"species": "Fe", "count": 1, "pos": [-4.0, 0.2, 0]},
                      {"species": "CuSO4", "count": 1}],
        "products": [{"species": "FeSO4", "count": 1},
                     {"species": "Cu", "count": 1}],
        "atom_map": [
            ["Fe#1.X", "FeSO4#1.Fe"],
            ["CuSO4#1.Cu", "Cu#1.X"],
            ["CuSO4#1.S", "FeSO4#1.S"],
            ["CuSO4#1.O1", "FeSO4#1.O1"],
            ["CuSO4#1.O2", "FeSO4#1.O2"],
            ["CuSO4#1.O3", "FeSO4#1.O3"],
            ["CuSO4#1.O4", "FeSO4#1.O4"],
        ],
        "electrons": [
            {"from": "Fe1", "to": "Cu1", "count": 2, "label": "e⁻"},
        ],
        "energy": {"activation": 0.2, "deltaH": -0.4,
                   "reactantLabel": "Fe+CuSO₄", "productLabel": "FeSO₄+Cu"},
        "steps": [
            {"title": "铁钉放入硫酸铜溶液",
             "html": "<span class='text-orange-300 font-semibold'>铁钉 (Fe)</span> 放入"
                     "<span class='text-blue-300 font-semibold'>硫酸铜 (CuSO₄)</span> 蓝色溶液中。"
                     "铁比铜活泼（金属活动性顺序中 Fe 在 Cu 之前），能<span class='text-emerald-400 font-semibold'>置换</span>出铜。"},
            {"title": "电子从铁转移到铜",
             "html": "Fe 失去 2 个 <span class='text-cyan-300 font-semibold'>电子</span> 变成 Fe²⁺（<span class='text-amber-400 font-semibold'>氧化</span>），"
                     "Cu²⁺ 得到 2 个电子变成 <span class='text-orange-300 font-semibold'>Cu 原子</span>（<span class='text-emerald-400 font-semibold'>还原</span>），"
                     "在铁钉表面<span class='text-orange-300 font-semibold'>析出红色铜</span>。溶液由蓝色渐变为浅绿色（Fe²⁺）。"},
            {"title": "生成硫酸亚铁与铜",
             "html": "铁溶解进入溶液形成 <span class='text-green-300 font-semibold'>FeSO₄（硫酸亚铁，浅绿色）</span>，"
                     "铜析出附着在铁钉表面。电子转移 2e⁻，原子守恒 ✓。"},
        ],
    }
...
REGISTRY = {
    ...
    "zn_hcl": build_zn_hcl,
    "na_water": build_na_water,
    "mg_burn": build_mg_burn,
    "fe_cuso4": build_fe_cuso4,
}
