...
授权脚本会优先读取包内 `_meta.json` 的 `slug` 作为服务端 `skillCode`。`slug` 必须是平台维护的技能编号，例如 `BT_ZYYXPPTHDPZZ_20260616`，不是 Skill 展示名称。只有包内缺少 `_meta.json` 时，才需要手动传 `--skill-code`。
...
3. 如果输出 `AUTH_REQUIRED`、`AUTH_INVALID`、`CREDENTIALS_MISSING` 或 `CREDENTIALS_INVALID`：
   - **必须立即调用** WorkBuddy 的 `connect_cloud_service` 工具获取当前登录用户 JWT（⚠️ WorkBuddy 可能因场景限制不会自动执行，你必须主动调用，不得跳过或假设用户已完成）。
   - 将 JWT 交给授权脚本完成绑定：
...
触发：`~/.workbuddy/skill_auth/BT_ZYYXPPTHDPZZ_20260616.json` 不存在。
...
（本地未找到本 Skill 的凭证缓存：~/.workbuddy/skill_auth/BT_ZYYXPPTHDPZZ_20260616.json）

【需要你完成以下步骤】
1. 我会立即调用 WorkBuddy 的 connect_cloud_service 工具获取当前登录用户 JWT（⚠️ 不会等待自动触发，我会主动调用）
2. 拿到 JWT 后自动执行 $PYTHON_CMD scripts/auth.py --bind-jwt "<JWT>" 完成绑定（$PYTHON_CMD 为检测到的 Python 命令，如 python/python3/py）
...
2. 我会立即调用 WorkBuddy 的 connect_cloud_service 获取新 JWT（⚠️ 不会等待自动触发，我会主动调用）
3. 拿到新 JWT 后自动重新绑定（会覆盖旧缓存），随后重新执行 --ensure
...
2. 或联系管理员核实 skillCode "BT_ZYYXPPTHDPZZ_20260616" 是否对你的账号开放
...
3. 若你认为不应出现此提示，请联系管理员核实 skillCode "BT_ZYYXPPTHDPZZ_20260616" 的权限配置
