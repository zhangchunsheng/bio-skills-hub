...
- 按 `hint` 提示用户：访问 `https://ai.zhiliaobiaoxun.com/?ch=s70` 用手机号登录（新号有赠送额度），
...
def get_api_key():
    ...
    if resp.status == 429:
        show_to_user("自动注册过于频繁，请访问 https://ai.zhiliaobiaoxun.com/?ch=s70 手动注册")
        return None, source=None
