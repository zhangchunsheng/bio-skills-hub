#!/usr/bin/env python3
"""
十万个为什么科普图文生成 - 执行任务接口（真实调用时使用）
调用方式: python scripts/execute_task.py --question "为什么海水不能喝？"
...
def execute_task(
    ...
    Args:
        question: 用户的科普问题（必填）
        aspect_ratio: 输出画幅比例，枚举：3:4(小红书/海报)、16:9(视频封面)、1:1(朋友圈)，默认3:4
        mode: 调用模式 async 或 sync，默认为 async
    ...
    # 2. 构建请求
    url = "https://route.showapi.com/flow/execute/6a6ff06a3c51f2ec21002bf8"
    headers = {
...
def main():
    parser = argparse.ArgumentParser(description="十万个为什么科普图文生成 - 执行任务")
    parser.add_argument("--question", "-q", required=True, help="用户的科普问题")
    parser.add_argument("--aspect-ratio", "-a", default="3:4", choices=["1:1", "3:4", "16:9"], help="输出画幅比例，默认3:4")
    parser.add_argument("--async", dest="mode", action="store_const", const="async", default="async", help="使用异步模式")
