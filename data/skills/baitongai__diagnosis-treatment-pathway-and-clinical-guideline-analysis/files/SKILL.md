---
name: "诊疗路径与临床指南分析（CSCO/NCCN/ESMO等）"
agent_created: true
---

# 诊疗路径与临床指南分析（CSCO/NCCN/ESMO等）
...
授权脚本会优先读取包内 `_meta.json` 的 `slug` 作为服务端 `skillCode`。`slug` 必须是平台维护的技能编号，例如 `BT_1379B5B7`，不是 Skill 展示名称。只有包内缺少 `_meta.json` 时，才需要手动传 `--skill-code`。
...
触发：`~/.workbuddy/skill_auth/BT_1379B5B7.json` 不存在。
...
（本地未找到本 Skill 的凭证缓存：~/.workbuddy/skill_auth/BT_1379B5B7.json）
...
2. 或联系管理员核实 skillCode "BT_1379B5B7" 是否对你的账号开放
...
3. 若你认为不应出现此提示，请联系管理员核实 skillCode "BT_1379B5B7" 的权限配置
