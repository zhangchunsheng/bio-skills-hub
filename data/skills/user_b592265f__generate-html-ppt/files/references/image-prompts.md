...
## 图片生成硬规则(禁水印 / 禁 chrome / 禁"原始截图"文字)

生成图本身**不能**包含以下任何元素,否则就是占位符,不是配图:

- ❌ "原始截图" / "产品截图" / "Screenshot" / "Untitled" / "Sample" / "Demo" 任何水印文字
- ❌ PPT 外壳: 页眉、页脚、页码、标题栏、角标、署名、装饰边框
- ❌ 假 logo 或不存在的品牌标识
- ❌ 操作系统 UI 元素(Windows 任务栏、Mac Dock、iOS 状态栏等),除非走 T5 Device Frame 范式
- ❌ 截图工具的悬浮工具条、选择框、马赛克、箭头标注(那是 Snipaste/QQ 截图的风格,不是 PPT 配图)
- ❌ 任何 "图 1" / "Chart 1" / "Slide 5" 之类自动编号

如果生成图里出现上述任何一项,**重新生成**,或在合成阶段用 T3 Inset / T7 Backdrop 把它藏到背景层。

## 比例选择

| 用途 | 推荐比例 | HTML 落位 | 范式 |
|------|---------|-----------|------|
| 章节封面 / 全屏主视觉 | 16:9 | `.t-bleed` 或 T7 Backdrop | T1 / T7 |
| 瑞士风顶部横幅 / Image Hero | 16:9 或 21:9 | `.t-bleed.r-21x9` | T1 |
| 左文右图主图 | 16:10 或 4:3 | `.t-float.r-16x10` / `.t-float.r-4x3` | T2 |
| 信息图 / 系统关系图 | 16:9 或 16:10 | `.t-inset.r-16x9` / `.t-inset.r-16x10` | T3 |
| 截图再设计 / UI 情景图 | 16:10 或 21:9 | `.t-inset.r-16x10` 或 `.t-inset.r-21x9` | T3 |
| 图文混排小图 | 3:2 或 3:4 | `.t-float.r-3x2` / `.t-float.r-3x4` | T2 |
| 图片网格 | 统一横图 | `.t-quiet` + `style="height:22vh\|26vh"` | T6 |
| 小型面板组 | 统一横图 | `.t-quiet` + `style="height:16vh\|18vh"` | T6 |

完整范式规范见 [`image-treatments.md`](image-treatments.md)。生成时按表格选范式 + 比例,不要回退到旧 `.frame-img` + `.img-cap` "原始截图"那套。
...
用于把真实截图、代码、设计稿、工作区处理成统一视觉素材。生成后用 T3 Inset 嵌入(`.t-inset.r-16x10`),无圆角无阴影,靠浅色色阶分层。**禁止在生成的图里写 "原始截图 / 产品截图 / Screenshot / Untitled / Sample" 等任何水印**。

```text
生成一张横向 UI 情景图,把[截图/界面/工作区内容]再设计成适合杂志风 PPT 的视觉。保留真实产品工作流的感觉,使用纸张底色、细线框、网格、少量标注,不要拟物阴影。图中文字使用[中文/英文],短而清晰。不要真实品牌 logo、花哨 dashboard、霓虹渐变、过度拟物或任何"原始截图 / 产品截图 / Screenshot / Untitled / Sample"水印。输出必须是16:10横向构图,主体居中但保留边距,画面密度中等。只保留核心 UI 画面本身,不要生成页眉、页脚、标题、页码、角标、署名、装饰边框、超长条、竖图或不规则比例,不要任何 PPT 外壳或截图工具的标注。
```
