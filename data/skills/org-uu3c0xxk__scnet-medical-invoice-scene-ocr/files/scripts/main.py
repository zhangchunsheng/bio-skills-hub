...
def print_upload_warning(file_path):
    """在上传文件前向 stderr 输出数据安全与外部传输警告。"""
    warning = (
        "\n"
        "===============================================\n"
        "⚠️  数据安全与外部传输警告\n"
        "===============================================\n"
        f"即将上传文件: {file_path}\n"
        "目标服务: Scnet OCR API (https://api.scnet.cn)\n"
        "\n"
        "医疗票据图片通常包含姓名、身份证号、社保卡号、医保号、\n"
        "诊断、住院信息及费用明细等敏感个人健康信息。\n"
        "\n"
        "继续运行即表示你确认：\n"
        "  1. 已阅读并理解 Scnet 的数据处理条款；\n"
        "  2. 已获得相关主体的授权或具有合法处理依据；\n"
        "  3. 使用符合适用的隐私、数据安全及医疗健康法律法规。\n"
        "===============================================\n"
    )
...
def recognize_with_retry(ocr_type, file_path, config, retry_count=0):
    ...
    # 在上传前输出数据安全警告
    print_upload_warning(file_path)
