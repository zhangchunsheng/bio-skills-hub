...
description: 支持医疗报销场景的票据识别，包括医疗住院发票和医疗费用结算单。用户必须明确指定本地文件路径并确认需要上传至 Scnet OCR API 后，方可触发本技能。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
> **⚠️ 数据安全与合规警告**
>
> 医疗住院发票和医疗费用结算单包含姓名、身份证号、社保卡号、医保号、诊断、住院信息、费用明细等高度敏感的个人健康信息（PHI/PII）。
>
> 使用本技能时，你指定的图片文件将被上传至第三方 OCR 服务提供商 **Scnet（https://api.scnet.cn）** 进行处理。请确保：
> 1. 你已阅读并理解 Scnet 的隐私政策与数据处理条款；
> 2. 你已获得票据相关主体的明确授权，或具有合法处理该数据的依据；
> 3. 你的使用符合适用的隐私保护、数据安全及医疗健康相关法律法规；
> 4. 不在未脱敏的情况下共享、转发或公开发送包含敏感信息的识别结果。
>
> 如不确定是否有权处理某份票据，请勿使用本技能。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• MEDICAL_INPATIENT_INVOICE（医疗住院发票）<br>• MEDICAL_EXPENSE_SETTLEMENT（医疗费用结算单） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/medical_invoice_scene_ocr/scripts/main.py MEDICAL_INPATIENT_INVOICE /path/to/test.jpg
```

### 在 AI 对话中使用

用户必须**明确表达识别意图**并提供**本地文件路径**，例如：

- “请使用医疗住院发票 OCR 识别 /Users/name/Downloads/test.jpg”
- “请帮我识别医疗费用结算单，文件路径是 /Users/name/Downloads/settlement.jpg”

AI 在触发本技能前，应：
1. 确认用户已了解数据将上传至 Scnet；
2. 确认用户提供的文件路径确实为本地医疗票据文件；
3. 获得用户明确同意后再执行。

**不应**仅因用户提到“发票”“结算单”“识别图片”等关键词就自动调用本技能。
